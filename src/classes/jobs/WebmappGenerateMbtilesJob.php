...
class WebmappGenerateMbtilesJob extends WebmappAbstractJob
{
    ...
    private function _generateMbtiles(string $instanceName, int $id, int $maxZoom = 16, int $minZoom = 4)
    {
        ...
        if ($this->verbose) {
            WebmappUtils::verbose("Generating map.mbtiles for {$instanceName}");
            WebmappUtils::verbose(" - Route ID    : $id");
            WebmappUtils::verbose(" - Min zoom    : $minZoom");
            WebmappUtils::verbose(" - Max zoom    : $maxZoom");
            WebmappUtils::verbose(" - MBTiles path: $mbtilesPath");
            WebmappUtils::verbose(" - Geojson path: $geojsonPath");
        }

        if ($this->verbose) {
            WebmappUtils::verbose("Cleaning temporary tables");
        }
        ...
        if ($this->verbose) {
            WebmappUtils::verbose("Using {$routeGeojsonUrl} file");
        }
        ...
        if (json_last_error() != JSON_ERROR_NONE) {
            ...
            if ($this->verbose) {
                WebmappUtils::verbose("Decrypting {$routeGeojsonUrl} into {$dest}");
            }
        ...
        if ($this->verbose) {
            WebmappUtils::verbose("Importing tracks to POSTGRES");
        }
        ...
        if ($this->verbose) {
            WebmappUtils::verbose("Creating the tiles buffer");
        }
        ...
        if ($this->verbose) {
            WebmappUtils::verbose("Creating the mbtiles file");
        }
        $cmd = "bash tlcopy.sh";
        system($cmd);
        $cmd = "bash bbox.sh";
        ...
        if ($this->verbose) {
            WebmappUtils::verbose("Cleaning up");
        }
