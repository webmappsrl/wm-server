...
$geojson_path = "/root/k.webmapp.it/{$instance}/geojson";
...
$cmd = "psql -U webmapp -d offline -h localhost -c \"create table tmp4 as SELECT concat('tl copy  -z 14 -Z {$max_zoom} -b \\\"', st_xmin(ST_Envelope(ST_transform(geom, 4326))), ' ', st_ymin(ST_Envelope(ST_transform(geom, 4326))), ' ', st_xmax(ST_Envelope(ST_transform(geom, 4326))), ' ', st_ymax(ST_Envelope(ST_transform(geom, 4326))), '\\\"  file:/root/api.webmapp.it/tiles file://./map') AS bbox from tmp3;\"";
system($cmd . " > /dev/null");
...
$cmd = "psql -U webmapp -d offline -h localhost -c \"create table tmp6 as SELECT concat('{\\\"minzoom\\\":{$min_zoom},\\\"maxzoom\\\":{$max_zoom},\\\"bounds\\\":[', tmp5.x_min, ',', tmp5.y_min, ',', tmp5.x_max, ',', tmp5.y_max, ']}') AS scommand from tmp5;\"";
system($cmd . " > /dev/null");
...
$cmd = "psql -U webmapp -d offline -h localhost -c \"DROP TABLE tmp6;\"";
system($cmd . " > /dev/null");
$cmd = "psql -U webmapp -d offline -h localhost -c \"create table tmp6 as SELECT concat('tl copy  -z {$min_zoom} -Z 13 -b \\\"',tmp5.x_min, ' ', tmp5.y_min, ' ', tmp5.x_max, ' ', tmp5.y_max, '\\\" file:/root/api.webmapp.it/tiles file://./map') AS scommand from tmp5;\"";
system($cmd . " > /dev/null");
